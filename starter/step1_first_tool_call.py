...
def main():
    ...
    # TODO(1): Claude only ASKED for the tool call above — nothing has
    # actually run yet. Your turn:
    #   1. Find the tool_use block in response.content
    #      (hint: block.type == "tool_use")
    #   2. Call find_overdue_bugs(FindOverdueBugsInput(**block.input))
    #   3. Print result.status.value and result.message
    #
    # Expected output once this works:
    #   status : SUCCESS
    #   message: 11 bug(s) have breached their SLA window. Priya Nair has
    #            the most, with 7.
    raise NotImplementedError("Your turn — see TODO(1) above")
